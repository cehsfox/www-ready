...
#
# $Id: mt.php.pre 413 2006-06-26 19:50:06Z jallen $

define('VERSION', '3.3');
define('VERSION_ID', '3.33');
define('PRODUCT_NAME', 'Movable Type Publishing Platform');
...
class MT {
    ...
    function init($blog_id = null, $cfg_file = null) {
        ...
        if (extension_loaded('mbstring')) {
            mb_internal_encoding($this->config['PublishCharset']);
            mb_http_output($this->config['PublishCharset']);
        }
    }
    ...
    function &db() {
        ...
        $this->db = new $mtdbclass($this->config['DBUser'],
            ...
            $this->config['DBHost'], $this->config['DBPort']);
        return $this->db;
    }
        ...
        if ($db_config) {
            ...
            if ($cfg['DBDriver'] == 'mysql' or $cfg['DBDriver'] == 'postgres') {
                if ($cfg['SQLSetNames']) {
                    ...
                    $lang = $Charset[$this->config['DBDriver']][strtolower($this->config['PublishCharset'])];
                    if ($lang) {
...
function translate_phrase_param($str, $params = null) {
    if (is_array($params) && strpos($str, '[_')) {
