...
define ('UNLVLIB_URL_EVENTS', 'https://cal.library.unlv.edu/1.1/events?cal_id=12058&category=47711&date=' . $date . '&days=365&limit=' . MAX_EVENTS);
...
//print_r ($events);
    ...
    foreach ( $events as $event )
    {
        ...
        // 2021-06-11, to fix a small glitch with feed.
        // If it's midnight, should be '12:00 am', not '0:00 am'.
        if ( '0:00 AM' == $start_time || '00:00 AM' == $start_time )
            $start_time = '12:00 AM';
        ...
        if ( '0:00 AM' == $end_time || '00:00 AM' == $end_time )
            $end_time = '12:00 AM';
    ...
    <div class="views-row">
        ...
        $return_str .= '
                <p>' . $start_time . ' - ' . $end_time . '<br>' . $location . '</p>
            </span>
        </div>';
...
// Sample $time_string: 2020-01-22T13:30:00-08:00
{
    ...
    if ( $date_pieces[2] < 10 )
        ...
        // Time part.
    $time_pieces = explode ('-', $date_time[1]);    // hh:mm:ss, 08:00
    $times = explode (':', $time_pieces[0]);       // hh, mm, ss
    $hour = $times[0];
    $minute = $times[1];
    ...
    $time_print = $hour . ':' . $minute . ' ' . $am_pm;


        // return array (yyyy,mm,d(d)).
    return array ($date_pieces[0], $date_pieces[1], $date_day, $time_print);
}
